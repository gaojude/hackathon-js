...
inventoryManager.post('/inventory/list-all', (req, res) => {
...
inventoryManager
    ...
    .post((req, res) => {
        ...
        InventoryItem.find({userId, name})
            ...
            .then((docs) => {
                if (docs.length === 0) {
                    new InventoryItem({userId, name, image, quantity}).save((err, doc) => {
    ...
    .delete((req, res) => {
        const {id} = req.body;
        InventoryItem.findByIdAndDelete(id).then((doc) => res.send(doc));
    })
    .patch((req, res) => {
